...
function dohvatiSveIzStorage() {
    ...
    const podaci =
        localStorage.getItem(PrefixStorage.SIREVI);
...
async function dodaj(sir) {
    ...
    if (sirevi.length === 0) {
        ...
        const maxId = Math.max(
            ...sirevi.map(s => parseInt(s.id))
        );

        sir.id = String(maxId + 1);
    }
